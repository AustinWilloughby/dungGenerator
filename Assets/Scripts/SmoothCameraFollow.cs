...
public class SmoothCameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        if (followTarget)
        {
            ...
            Vector3 current = GetComponent<Camera>().WorldToViewportPoint(followTarget.transform.position);
            Vector3 change = followTarget.transform.position - GetComponent<Camera>().ViewportToWorldPoint(new Vector3(.5f, .5f, current.z));
